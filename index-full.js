...
A0.ShowPage = function ( n = "" ) {
	...
	if ( typeof n === "string" ) {
		...
		// console.log("[App ] ShowPage:", n );
		if ( ! found ) {
			console.warn( "A0.ShowPage >> Not found:", pg_id );
...
A0.Storage.Clear = function () {
	...
	console.warn( "TODO: A0.Storage.Clear" );
};
...
A0.Storage.PopulateItemJSON = function ( id, values ) {
	console.log( "A0.Storage.PopulateItemJSON >>", id, values );
	...
	if ( data === null ) {
		...
		for ( var key in data ) {
			if ( values.hasOwnProperty( key ) ) {
				...
				console.error( "A0.Storage.PopulateItemJSON >> No such key:", key );
			}
		};
	}
...
class CardGame {
	...
	#onClick ( js_entity, evt_target ) {
		...
		if ( this.#game_over ) {
			console.warn( "onClick >> Game is over" );
			return;
		}
	...
	#onClickCard ( card ) {
		console.log( "onClickCard >>", card.GetId() );
		...
		if ( card !== null ) {
			...
			if ( status !== 0 ) {
				...
				if ( card.IsBlack() ) {
					...
					console.assert( this.#hand.GetNrCards() >= 2 );
	...
	#onClickMonster ( card ) {
		console.log( "onClickMonster >>", card.GetValue() );
	...
	#onClickPotion ( card ) {
		console.log( "onClickPotion>>", card.GetValue() );
	...
	#onClickWeapon ( card ) {
		console.log( "onClickWeapon >>", card.GetValue() );
	...
	#Start () {
		...
		this.#MakeDrawPile();
		console.assert( this.#deck.GetNrCards() === 0 );
	...
	#RemoveCardFromDungeon ( card, target, new_round_if_needed = true  ) {
		console.assert( card.GetLocation() === this.#dungeon );
		console.assert( target === this.#hand || target === this.#discard );
	...
	#NewRoundIfNeeded () {
		if ( this.#game_over ) {
			console.log( "NewRoundIfNeeded >> Game is over" );
			return;
		}
...
class CardsRow extends FlexBox {
	...
	onResize ( width, height, top, left ) {
		// console.log( "Hand::onResize >>", width, "x", height, "@", top, left );
		super.onResize( width, height, top, left );
		// this.SetEntitiesPosition();
	}
	...
	_Status () {
		console.assert( this.#is_hand );
		console.log( "Hand >> weapon:", this.#weapon, "enemy:", this.#monster );
	}
	...
	TestAddCard ( card ) {
		...
		if ( this.#lst_cards.length === 0 ) {
			...
			if ( ! card.IsDiamonds() ) {
				// console.warn( "Hand:AddCard >> First card not a weapon:", card );
				return 1;
			}
		} else {
			...
			console.assert( this.#weapon > 0 );
			if ( ! card.IsBlack() ) {
				// console.warn( "Hand:AddCard >> 2+ card not a enemy:", card );
				return 2;
			} else if ( card.GetValue() >= this.#monster ) {
				// console.warn( "Hand:AddCard >> Monster value failed:", card.GetValue(), this.#monster, card );
				return ( - this.#monster );
			}
		}
	...
	RemoveLastCard () {
		// console.assert( ! this.#is_hand );
		console.assert( this.#lst_cards.length > 0 );
...
class Stats {
    ...
    UpdateHealth ( value ) {
        console.log( "Stats::UpdateHealth >>", value );
    ...
    UpdateHistory ( win, score ) {
        ...
        console.log( "-- Stats:", win, score );
        console.log( "Finished:", this.#data.games_finished );
        console.log( "Won:", this.#data.games_won );
        console.log( "Win ratio:", Math.round( this.#data.games_won / this.#data.games_finished * 100) );
        console.log( "Latest:", this.#data.lst_last_scores );
        console.log( "Best:", this.#data.lst_best_scores );
    ...
    #UpdateFromStorage ( storage_data ) {
        for ( var key in storage_data ) {
            if ( this.#data.hasOwnProperty( key ) ) {
                ...
                console.error( "Stats::UpdateFromStorage >> No such key:", key );
            }
        }
    }
